...
/**
 * Класс представляет команду обратного контроля (по определнному QR-коду (сигналу от сканера кодов) изменить что-либо в приложении)
 * @author Mikhail Zakharov <zmish93@ya.ru>
 * @since 0.0.1
 */
public class BackControlBarCodeCommand extends BarCodeCommand {

    /**
     * Константа определяет тип данной команды
     */
    private static final TypeOfBarCodeCommand type = TypeOfBarCodeCommand.BACK_CONTROL;
    /**
     * Поле, содержащее строку, определяющую действие данной команды
     */
    private String action;
    /**
     * Поле содержит хэш-таблицу (название/значение), содержащую параметры данной команды
     */
    private final HashMap<String, String> parameters = new HashMap<String, String>(0);
    
    /**
     * Конструктор позволяет создать команду обратного управления с указанием необходимого действия
     * @param action Действие команды
     */
    public BackControlBarCodeCommand(String action) {
        this.action = action;
    }
    
    /**
     * Метод позволяет создать команду обратного управления из строки
     * @param str Строка, представляющая команду обратного управления
     * @return Созданная команда обратного управления
     * @throws ParseException Исключение бросается, если переданная строка не представляет команду обратного управления или если она [строка] 
     * некорректна
     */
    public static BarCodeCommand parseFromString(String str) throws ParseException {
        if (!XMLHelper.getXmlElements(str).get(0).getElementName().equals(String.valueOf(type.toChar()))) {
            throw new ParseException("It is not a back control command", 0);
        }
        ...
        BackControlBarCodeCommand command = new BackControlBarCodeCommand(attribute.getValue());
     ...
     * @return Тип данной команды (BACK_CONTROL)
     */
    @Override
     ...
     * Примеры: 
     * {@code <b a="act"><p n="key">value</p><p n="key2">value2</p></b>}, 
     * {@code <b a="0"/>}
     */
    @Override
    ...
    @Override
    public void execute() {
        throw new UnsupportedBarCodeOperationException("Not supported execution of back control command");
    }
