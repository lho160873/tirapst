 ...
 */
package pst.arm.client.common.test;
...
import pst.arm.client.common.test.model.BaseDto;
import pst.arm.client.common.test.model.Country;
import pst.arm.client.common.test.model.Data;
import pst.arm.client.common.test.model.FolderDto;
import pst.arm.client.common.test.model.MailItem;
import pst.arm.client.common.test.model.Movies;
import pst.arm.client.common.test.model.MusicDto;
import pst.arm.client.common.test.model.Plant;
import pst.arm.client.common.test.model.State;
import pst.arm.client.common.test.model.Stock;
import pst.arm.client.common.test.model.Task;
import pst.arm.client.common.test.model.TeamSales;
...
public class TestData {
  ...
  private static final String[] senders = new String[] {
      "markboland05", "Hollie Voss", "boticario", "Emerson Milton", "Healy Colette", "Brigitte Cobb", "Elba Lockhart",
      "Claudio Engle", "Dena Pacheco", "Brasil s.p", "Parker", "derbvktqsr", "qetlyxxogg", "antenas.sul",
      "Christina Blake", "Gail Horton", "Orville Daniel", "PostMaster", "Rae Childers", "Buster misjenou", "user31065",
      "ftsgeolbx", "aqlovikigd", "user18411", "Mildred Starnes", "Candice Carson", "Louise Kelchner",
      "Emilio Hutchinson", "Geneva Underwood", "Residence Oper?", "fpnztbwag", "tiger", "Heriberto Rush",
      "bulrush Bouchard", "Abigail Louis", "Chad Andrews", "bjjycpaa", "Terry English", "Bell Snedden", "huang", "hhh",
      "(unknown sender)", "Kent", "Dirk Newman", "Equipe Virtual Cards", "wishesundmore", "Benito Meeks"};

  private static final String[] emails = new String[] {
      "mark@example.com", "hollie@example.com", "boticario@example.com", "emerson@example.com", "healy@example.com",
      "brigitte@example.com", "elba@example.com", "claudio@example.com", "dena@example.com", "brasilsp@example.com",
      "parker@example.com", "derbvktqsr@example.com", "qetlyxxogg@example.com", "antenas_sul@example.com",
      "cblake@example.com", "gailh@example.com", "orville@example.com", "post_master@example.com",
      "rchilders@example.com", "buster@example.com", "user31065@example.com", "ftsgeolbx@example.com",
      "aqlovikigd@example.com", "user18411@example.com", "mildred@example.com", "candice@example.com",
      "louise_kelchner@example.com", "emilio@example.com", "geneva@example.com", "residence_oper@example.com",
      "fpnztbwag@example.com", "tiger@example.com", "heriberto@example.com", "bulrush@example.com",
      "abigail_louis@example.com", "chada@example.com", "bjjycpaa@example.com", "terry@example.com",
      "bell@example.com", "huang@example.com", "hhh@example.com", "kent@example.com", "newman@example.com",
      "equipe_virtual@example.com", "wishesundmore@example.com", "benito@example.com"};
  private static final String[] subjects = new String[] {
      "URGENT -[Mon, 24 Apr 2006 02:17:27 +0000]", "URGENT TRANSACTION -[Sun, 23 Apr 2006 13:10:03 +0000]",
      "fw: Here it comes", "voce ganho um vale presente Boticario", "Read this ASAP", "Hot Stock Talk",
      "New Breed of Equity Trader", "FWD: TopWeeks the wire special pr news release", "[fwd] Read this ASAP",
      "Renda Extra R$1.000,00-R$2.000,00/m?s", "re: Make sure your special pr news released",
      "Forbidden Knowledge Conference", "decodificadores os menores pre?os", "re: Our Pick",
      "RE: The hottest pick Watcher", "RE: St0kkMarrkett Picks Trade watch special pr news release",
      "St0kkMarrkett Picks Watch special pr news release news", "You are a Winner oskoxmshco",
      "Encrypted E-mail System (VIRUS REMOVED)", "Fw: Malcolm", "Secure Message System (VIRUS REMOVED)",
      "fwd: St0kkMarrkett Picks Watch special pr news releaser",
      "FWD: Financial Market Traderr special pr news release", "? s? uma dica r?pida !!!!! leia !!!",
      "re: You have to heard this", "fwd: Watcher TopNews", "VACANZE alle Mauritius", "funny",
      "re: You need to review this", "[re:] Our Pick", "RE: Before the be11 special pr news release",
      "[re:] Market TradePicks Trade watch news", "No prescription needed", "Seu novo site",
      "[fwd] Financial Market Trader Picker", "FWD: Top Financial Market Specialists Trader interest increases",
  ...
  private static final String[] fragments = new String[] {
      ...
      "I have an urgent and very confidential business proposition for you. On "
          ...
          + "calendar months, valued at GBP?30, 000,000.00 (Thirty Million Pounds " + "only) in my Branch.",
      ...
      "The OIL sector is going crazy. This is our weekly gift to you!<br>" + "<br>"
          + "Get KKPT First Thing, This Is Going To Run!<br>" + "<br>" + "Check out Latest NEWS!<br>" + "<br>"
      ...
      "JMT has contracted with Well Enhancement Services, LLC (www."
          ...
          + "reservoir. Total cost of the re-work per well is estimated to be " + "approximately $50,000 USD.",
      ...
      "Large Marketing Campaign running this weekend!<br>" + "<br>" + "Should you get in today before it explodes?<br>"
      ...
      "PREMIER INFORMATION (PIFR)<br>" + "A U.S. based company offers specialized information management "
          ...
          + "we provide are specific to each industry and designed for quick " + "response and maximum security.<br>"
          + "<br>" + "STK- PIFR<br>" + "Current Price: .20<br>"
      ...
      ">> obrigado por me dar esta pequena aten??o !!!<br>"
          ...
          + "ASSISTE A MUITOS CANAIS , FILMES , JOGOS , PORNOS , DESENHOS , " + "DOCUMENT?RIOS ,SHOWS , ETC,<br><br>"
      ...
      "********************************************************************<br>" + " Original filename: mail.zip<br>"
          ...
          + " by the virus and not a person - treat this message as you would<br>" + " any other junk mail (spam).<br>"
  ...
  public static List<Data> getDataNegative(int size, double min, double scale) {
    ...
    for (int i = 0; i < size; i++) {
      data.add(new Data(monthsFull[i % size], Math.floor(Math.min(Math.random() * scale, min)), Math.floor(Math.min(Math.random() * scale, min)),
          ...
          Math.floor(Math.min(Math.random() * scale, min)), Math.floor(Math.min(Math.random() * scale, min)), Math.floor(Math.min(Math.random() * scale, min)),
          Math.floor(Math.min(Math.random() * scale, min))));
    }
    return data;
  }
  ...
  public static List<Movies> getMovieData(int firstYear, int size, double min, double scale) {
  ...
  public static List<Stock> getShortStocks() {
    ...
    stocks.add(new Stock("Apple", "AAPL", 125.64, 123.43, randomDate()));
    stocks.add(new Stock("Cisco Systems", "CSCO", 25.84, 26.3, randomDate()));
    stocks.add(new Stock("Google Inc.", "GOOG", 516.2, 512.6, randomDate()));
    stocks.add(new Stock("Intel", "INTC", 21.36, 21.53, randomDate()));
    stocks.add(new Stock("Microsoft", "MSFT", 29.56, 29.72, randomDate()));
    stocks.add(new Stock("Nokia (ADR)", "NOK", 27.83, 27.93, randomDate()));
    stocks.add(new Stock("Oracle", "ORCL", 18.73, 18.98, randomDate()));
    stocks.add(new Stock("Starbucks", "SBUX", 27.33, 27.36, randomDate()));
    stocks.add(new Stock("Yahoo! Inc.", "YHOO", 26.97, 27.29, randomDate()));
    stocks.add(new Stock("Comcast", "CMCSA", 25.9, 26.4, randomDate()));
  ...
  public static List<Task> getTasks() {
    ...
    tasks.add(new Task(100, "Ext Forms: Field Anchoring", 112, "Integrate 2.0 Forms with 2.0 Layouts", 6, 150,
        "06/24/2007"));
    tasks.add(new Task(100, "Ext Forms: Field Anchoring", 113, "Implement AnchorLayout", 4, 150, "06/25/2007"));
    tasks.add(new Task(100, "Ext Forms: Field Anchoring", 114, "Add support for multiple types of anchors", 4, 150,
        "06/27/2007"));
    tasks.add(new Task(100, "Ext Forms: Field Anchoring", 115, "Testing and debugging", 8, 0, "06/29/2007"));

    tasks.add(new Task(101, "Ext Grid: Single-level Grouping", 101, "Add required rendering 'hooks' to GridView", 6,
        100, "07/01/2007"));
    tasks.add(new Task(101, "Ext Grid: Single-level Grouping", 102, "Extend GridView and override rendering functions",
        4, 0, "07/03/2007"));
    tasks.add(new Task(101, "Ext Grid: Single-level Grouping", 103, "Extend Store with grouping functionality", 4, 100,
        "07/04/2007"));
    tasks.add(new Task(101, "Ext Grid: Single-level Grouping", 121, "Default CSS Styling", 2, 1000, "07/05/2007"));
    ...
    tasks.add(new Task(102, "Ext Grid: Summary Rows", 106, "Summary creation during rendering phase", 6, 125,
        "07/02/2007"));
    tasks.add(new Task(102, "Ext Grid: Summary Rows", 107, "Dynamic summary updates in editor grids", 6, 125,
        "07/05/2007"));
    tasks.add(new Task(102, "Ext Grid: Summary Rows", 108, "Remote summary integration", 4, 125, "07/05/2007"));
    tasks.add(new Task(102, "Ext Grid: Summary Rows", 109, "Summary renderers and calculators", 4, 125, "07/06/2007"));
    tasks.add(new Task(102, "Ext Grid: Summary Rows", 110, "Integrate summaries with GroupingView", 10, 125,
        "07/11/2007"));
  ...
  public static List<TeamSales> getTeamSales() {
    ...
    for (String m : monthsAbbreviated) {
      int upper = 50;
