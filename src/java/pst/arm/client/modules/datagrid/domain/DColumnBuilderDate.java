 ...
 * @since 0.0.1
 */
public class DColumnBuilderDate extends IColumnBuilder implements Serializable
{
    ...
    public HashMap<SKeyForColumn, Field> createFields(LayoutContainer fieldSet, GWTDDataGridServiceAsync service, Boolean isEnabled, Integer lw ) {
        ...
        for (Map.Entry colEntry : this.getColumns().entrySet()) {
            ...
            String sc = this.getColumns().get(key).getSubCaption();
            if (sc != null)
                field.setData("subCaptionText", sc);
    ...
    public String getSqlWhereSearch(SKeyForColumn key, String val) {
        ...
        if (this.getColumns().get(key).getColumnProperty().getFormat().equals("dd.MM.yyyy")) {
