 ...
 * @author wesStyle
 */
public class DColumnBuilderMultiForContract extends DColumnBuilderMulti{
    ...
    protected String queryNameForRelationNDS; //имя таблицы для получения доп. данных
    protected SKeyForColumn keyRelationForIdNDS; //ключ к полю, которое хотим отображать из queryNameForRelation в выпадающем списке
    protected SKeyForColumn keyRelationForNameNDS;//ключ к полю, которое хотим помещать в данные
    ...
    public String getQueryNameForRelationNDS() {
        return queryNameForRelationNDS;
    }

    public void setQueryNameForRelationNDS(String queryNameForRelationNDS) {
        this.queryNameForRelationNDS = queryNameForRelationNDS;
    }

    public SKeyForColumn getKeyRelationForIdNDS() {
        return keyRelationForIdNDS;
    }

    public void setKeyRelationForIdNDS(SKeyForColumn keyRelationForIdNDS) {
        this.keyRelationForIdNDS = keyRelationForIdNDS;
    }

    public SKeyForColumn getKeyRelationForNameNDS() {
        return keyRelationForNameNDS;
    }

    public void setKeyRelationForNameNDS(SKeyForColumn keyRelationForNameNDS) {
        this.keyRelationForNameNDS = keyRelationForNameNDS;
    }
    ...
    public void initData(GWTDDataGridService service) {
        try {
            List<DDataGrid> listData = service.getAllDataGrid(queryNameForRelationNDS);
            createList(listData, new SKeyForColumn("MAIN:NDS_ID"));
    ...
    public HashMap<SKeyForColumn, Field> createFields(LayoutContainer fieldSet, final GWTDDataGridServiceAsync service, Boolean isEnabled, Integer lw) {
        ...
        for (Map.Entry colEntry : this.getColumns().entrySet()) {
            ...
            if (key.equals(new SKeyForColumn("CONTRACT:CONTRACT_NUMB"))) {
            ...
            } else if (key.equals(new SKeyForColumn("MAIN:NDS_ID")) || key.equals(new SKeyForColumn("MAIN:FUNDING_SOURCE_ID"))) {
                flds.add(field);
            ...
            if (!(key.equals(new SKeyForColumn("MAIN:NDS_ID")) || key.equals(new SKeyForColumn("MAIN:FUNDING_SOURCE_ID"))) && (
    ...
    public void updateDataAndField(final SKeyForColumn key, final Field field, GWTDDataGridServiceAsync service) {
        ...
        if (key.equals(new SKeyForColumn("MAIN:NDS_ID")))
            service.getAllDataGrid(queryNameForRelationNDS, callback);
        else
    ...
    protected void createList(List<DDataGrid> listData, SKeyForColumn cKey) {
        ...
        if (cKey.equals(new SKeyForColumn("MAIN:NDS_ID"))) {
            for (DDataGrid domain : listData) {
                HashMap<SKeyForColumn, IRowColumnVal> rows = domain.getRows();
                if (rows.containsKey(keyRelationForIdNDS) && rows.containsKey(keyRelationForNameNDS)) {
                    list.add(new SListVal(rows.get(keyRelationForIdNDS).getVal(), rows.get(keyRelationForNameNDS).getVal()));
                }
            }
        } else {
            for (DDataGrid domain : listData) {
