 ...
 * @since 0.0.1
 */
public class DColumnBuilderMultiForDepartList extends IColumnBuilder implements Serializable {
    ...
    public void initData(GWTDDataGridService service) {
        try {
             List<DDataGrid> listData = service.getAllDataGrid("NTO_VO");
            createList(listData, new SKeyForColumn("MAIN:DEPART_ID"));
            isInit = true;
        } catch (Exception ex) {
            isInit = false;
        }
    ...
    protected void createList(List<DDataGrid> listData, SKeyForColumn cKey) {
        ...
        if (cKey.equals(new SKeyForColumn("MAIN:DEPART_ID"))) {
    ...
    protected void setValToFields(HashMap<SKeyForColumn, Field> fields, HashMap<SKeyForColumn, IRowColumnVal> rows, DTable table) {
        for (Map.Entry colEntry : relations.entrySet()) {
            ...
            if (key.equals(new SKeyForColumn("MAIN:DEPART_ID"))) {
                departId = String.valueOf(rows.get(key_r).getFormatVal(key_r, table.getColumnBuilder(key_r)));
            }
            else if (key.equals(new SKeyForColumn("DEPART:SHORT_NAME"))) {
                departShortName = String.valueOf(rows.get(key_r).getFormatVal(key_r, table.getColumnBuilder(key_r)));
                ((SimpleComboBox) fields.get(new SKeyForColumn("MAIN:DEPART_ID"))).setRawValue(departShortName);
            }
            else
        ...
        */
        ...
        for (Map.Entry colEntry : this.getColumns().entrySet()) {
            ...
            if (field instanceof SimpleComboBox) {
                ((SimpleComboBox)field).setEditable(false);
            }
    ...
    public void setDomainValueToField(SKeyForColumn key, Field field, DDataGrid domain) {
        ...
        if (field.getClass() == SimpleComboBox.class) {
            Integer id = (Integer) domain.getRows().get(new SKeyForColumn("MAIN:DEPART_ID")).getVal();
            String sn = (String) domain.getRows().get(new SKeyForColumn("DEPART:SHORT_NAME")).getVal();

            departId = id.toString();
            departShortName = sn;

            ((SimpleComboBox) field).setRawValue(String.valueOf(sn));
    ...
    public void setValueFromField(SKeyForColumn key, Field field, IRowColumnVal val) {
         if (field.getClass() == SimpleComboBox.class) {
             ...
             if (departShortName != null && departId != null) {
                 if (((SimpleComboBox)field).getSelectedIndex() == -1 && field.getRawValue().equals(departShortName)) {
                     val.setVal(departId);
                     //val.setValFromFormat(key, departId, this);
                 }
                 else {
