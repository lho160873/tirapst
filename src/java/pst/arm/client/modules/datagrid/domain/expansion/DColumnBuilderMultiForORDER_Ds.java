 ...
 * @author wesStyle
 */
public class DColumnBuilderMultiForORDER_Ds extends DColumnBuilderMulti{
    ...
    public HashMap<SKeyForColumn, Field> createFields(LayoutContainer fieldSet, final GWTDDataGridServiceAsync service, Boolean isEnabled, Integer lw) {
        ...
        for (Map.Entry colEntry : this.getColumns().entrySet()) {
            ...
            if (key.equals(new SKeyForColumn("MAIN:NOTIF_OPENING_ID"))) {
            ...
            } else if (key.equals(new SKeyForColumn("MAIN:COMPANY_ID"))) {
                flds.add(field);
