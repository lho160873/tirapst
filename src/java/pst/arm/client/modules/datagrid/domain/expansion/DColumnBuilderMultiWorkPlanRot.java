 ...
 * @since 0.0.1
 */
public class DColumnBuilderMultiWorkPlanRot extends IColumnBuilder implements Serializable {
    ...
    protected void setValToFields(HashMap<SKeyForColumn, Field> fields, HashMap<SKeyForColumn, IRowColumnVal> rows, DTable table) {
            ...
            Double lab = (Double) fields.get(new SKeyForColumn("MAIN:PLAN_LABOUR")).getValue();
            if (lab != null) {
                fields.get(new SKeyForColumn("MAIN:PLAN_COST")).setValue((Double)rows.get(new SKeyForColumn("MAIN:AVERAGE_SALARY")).getVal() * lab);
            } else {
                if (!alertShown) {
                    MessageBox.alert("Внимание", "Введите сначала значение в поле «Трудоемкость»", null);
                    alertShown = true;
                }
            }
    }
    ...
    public HashMap<SKeyForColumn, Field> createFields(LayoutContainer fieldSet, GWTDDataGridServiceAsync service, Boolean isEnabled,Integer lw ) {
        ...
        Button bOpenDataGrid = new Button("Рассчитать", AbstractImagePrototype.create(images.book()), buttonEventListener);
        ...
        */
        ...
        for (Map.Entry colEntry : this.getColumns().entrySet()) {
            ...
            if (isAddBtn || (!isAddBtn && !this.getColumn(key).getIsVisibleEdit())) {
                ...
                if (!key.equals(new SKeyForColumn("MAIN:PLAN_COST"))) {
    ...
    public HashMap<SKeyForColumn, Field> createFields(int companyId, LayoutContainer fieldSet, GWTDDataGridServiceAsync service, Boolean isEnabled,Integer lw, Boolean isAdd) {
        DataGridConstants constants = GWT.create(DataGridConstants.class);
        DataGridImages images = GWT.create(DataGridImages.class);
        final DataGridWindow page;

        IRowColumnVal val = new DRowColumnValNumber();
        val.setVal(companyId);

        page = new DataGridWindow(val, queryNameForRelation, isViewOnly, true);
        ...
        */
        ...
        for (Map.Entry colEntry : this.getColumns().entrySet()) {
            ...
            if (!key.equals(new SKeyForColumn("MAIN:PLAN_COST"))) {
                ...
                HBoxLayoutData dataBtn = new HBoxLayoutData();
                dataBtn.setMargins(new Margins(0, 32, 0, 21));
                dataBtn.setFlex(0);
