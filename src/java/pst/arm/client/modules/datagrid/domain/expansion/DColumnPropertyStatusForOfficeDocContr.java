 ...
 * @since 0.0.1
 */
public class DColumnPropertyStatusForOfficeDocContr extends DColumnPropertyTextField {
    ...
    public String valueToString( DDataGrid domain, String value) {
        DDataGrid row = domain;
        //SKeyForColumn keyFact = new SKeyForColumn(property);

        SKeyForColumn key = new SKeyForColumn("MAIN:DATE_FACT_STAT");


        SKeyForColumn keyFact = new SKeyForColumn("MAIN:DATE_FACT");
        SKeyForColumn keyPlan = new SKeyForColumn("MAIN:DATE_PLAN");
            ...
            if (((plan.after(curr)) || (plan.equals(curr))) && (fact == null)) {
               return "Назначено. Срок выполнения не прошел";
            } else if ((plan.before(curr)) && (fact == null)) {
                return "Не выполнено. Срок исполнения прошел";
            } else if (((plan.after(fact)) || (plan.equals(fact)))) {
