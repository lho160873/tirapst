 ...
 * @author LKHorosheva
 */
public class DataGridEditWindowDtJobPlanPriboy extends DataGridEditWindow {
    ...
    public DataGridEditWindowDtJobPlanPriboy(DDataGrid domain, DTable table, EditState state, EWindowType windowType) {
        super(domain, table, state, windowType);
        initTableOdpt();
    }

    public DataGridEditWindowDtJobPlanPriboy(DDataGrid domain, DTable table, EditState state, EWindowType windowType, DCondition cnd) {
        super(domain, table, state, windowType, cnd);
        initTableOdpt();
    }

    public DataGridEditWindowDtJobPlanPriboy(DDataGrid domain, DTable table, EditState state, EWindowType windowType, DCondition cnd, String customHeader) {
    ...
    protected final void initTableOdpt() {
        ...
        service2.getAllDataGrid("DEPART_FOR_PMASC", new AsyncCallback<List<DDataGrid>>() {
            ...
            public void onSuccess(List<DDataGrid> listData) {
                final DataBasePanel panelTableOdpt = DataGridPanelBuilder.createDataGridPanel("SPR_DEPART_PROBOY_HLV");
