 ...
 * @author LKhorosheva
 */
public class DataGridPanelDocFileElaboration extends DataGridPanel {

  
    final GWTDataGridFileOpenServiceAsync fileOpenService = GWT.create(GWTDataGridFileOpenService.class);

    protected Button btnCancell;
    protected MenuItem menuItemCancell;

    protected Button btnOpenFile;
    protected MenuItem menuOpenFile;
    
   protected Boolean isCancell = null, isOpenFile=null;
    
    public DataGridPanelDocFileElaboration(DTable table, String tn, DCondition cnd) {
        super(table, tn, cnd);
    }

    public DataGridPanelDocFileElaboration(DTable table, String tn) {
        super(table, tn);
    }

    public DataGridPanelDocFileElaboration(String tableName) {
        super(tableName);
    }

    public DataGridPanelDocFileElaboration(String tableName, SRelationInfo relationInfo) {
        super(tableName, relationInfo);
    }

    public DataGridPanelDocFileElaboration(DTable table, String tableName, SRelationInfo relationInfo) {
        ...
        final AsyncCallback<List<DDataGrid>> callback_getNorm = new AsyncCallback<List<DDataGrid>>() {
            ...
            public void onSuccess(List<DDataGrid> result) {
                if (result.size() != 1) {
                    ...
                    IRowColumnVal val = new DRowColumnValNumber();
                    if (table.getQueryName().equals("DOC_FILE_DH_ELABORATION_OF_DD_HLV")) {
                        val.setVal(998);
                    } else if (table.getQueryName().equals("DOC_FILE_APP_PRODUCTION_HLV")) {
                        val.setVal(997);
                    } else if (table.getQueryName().equals("DOC_FILE_LEVEL_TASK_HLV")) {
                        val.setVal(996);
                    }
                    ...
                    if (condition.getFilters().size() > 0) {
                        ...
                        for (Map.Entry filtr : condition.getFilters().entrySet()) {
                            ...
                            val = (IRowColumnVal) filtr.getValue();
                            domain.getRows().put(key, val);
                        }
                    }
                    ...
                    if (isOpenAsSub && passedFields != null) {
                        for (Map.Entry passField : passedFields.entrySet()) {
                            ...
                            //SKeyForColumn val = (SKeyForColumn) key.getValue();
                            val = (IRowColumnVal) passField.getValue();
                            domain.getRows().put(key, val);
                        }
                    }
                    ...
                    if (condition.getValDefault().size() > 0) {
                        ...
                        for (Map.Entry defaultVal : condition.getValDefault().entrySet()) {
                            ...
                            //SKeyForColumn val = (SKeyForColumn) key.getValue();
                            val = (IRowColumnVal) defaultVal.getValue();
                            domain.getRows().put(key, val);
                        }
                    }
