 ...
 * @author Igor
 */
public class DataGridPanelMassMailingIg extends DataGridPanel {

    public DataGridPanelMassMailingIg(DTable table, String tn, DCondition cnd) {
        super(table, tn, cnd);
    }

    public DataGridPanelMassMailingIg(DTable table, String tn) {
        super(table, tn);
    }

    public DataGridPanelMassMailingIg(String tableName) {
        super(tableName);
    }

    public DataGridPanelMassMailingIg(String tableName, SRelationInfo relationInfo) {
        super(tableName, relationInfo);
    }

    public DataGridPanelMassMailingIg(DTable table, String tableName, SRelationInfo relationInfo) {
    ...
    protected ToolBar createToolBar() {
        ...
        btnAddWithCopy = new Button(datagridConstants.btnAddWithCopy());
        btnAddWithCopy.setIcon(AbstractImagePrototype.create(commonImages.add()));
        btnAddWithCopy.setIconAlign(Style.IconAlign.LEFT);
        btnAddWithCopy.addSelectionListener(tbListener);
        menuItemAddWithCopy = new MenuItem(datagridConstants.btnAddWithCopy());
        menuItemAddWithCopy.setIcon(AbstractImagePrototype.create(commonImages.add()));
        menuItemAddWithCopy.addSelectionListener(new SelectionListener<MenuEvent>() {
            @Override
            public void componentSelected(MenuEvent ce) {
                onAddWithCopy();
            }
    ...
    protected void initListeners() {
        tbListener = new SelectionListener<ButtonEvent>() {
            ...
            public void componentSelected(ButtonEvent ce) {
                ...
                } else if (ce.getButton() == btnAddWithCopy) {
                    onAddWithCopy();
...
//        Integer massMailingId = Integer.valueOf(domain.getRows().get(new SKeyForColumn("MAIN:MASS_MAILING_ID")).toString());
        
//        DCondition cnd = new DCondition();
        SKeyForColumn key = new SKeyForColumn("MAIN:MASS_MAILING_ID");
//        cnd.setKey(key);
//        cnd.setVal(condition.getFilters().get(new SKeyForColumn("MAIN:MASS_MAILING_ID")).getVal().toString());
        
//        Integer massMailingId = Integer.valueOf(condition.getFilters().get(key).getStringFromVal(key, table.getColumnBuilder(key)));
        Integer massMailingId = (Integer) currentDomain.getRows().get(new SKeyForColumn("MAIN:MASS_MAILING_ID")).getVal();
        DataGridEditWindowMassMailingIg window = new DataGridEditWindowMassMailingIg(domain, table, Editable.EditState.NEW, EWindowType.EWT_WINDOW, massMailingId);
