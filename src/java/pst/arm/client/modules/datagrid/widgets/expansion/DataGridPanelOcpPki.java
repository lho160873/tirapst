 ...
 * @author igor
 */
public class DataGridPanelOcpPki extends DataGridPanel {

    private Button btnSummary;

    public DataGridPanelOcpPki(DTable table, String tn, DCondition cnd) {
        super(table, tn, cnd);
    }

    public DataGridPanelOcpPki(DTable table, String tn) {
        super(table, tn);
    }

    public DataGridPanelOcpPki(String tableName) {
        super(tableName);
    }

    public DataGridPanelOcpPki(String tableName, SRelationInfo relationInfo) {
        super(tableName, relationInfo);
    }

    public DataGridPanelOcpPki(DTable table, String tableName, SRelationInfo relationInfo) {
...
//                showReport();
                            ...
                            final AsyncCallback<List<DDataGrid>> callback_getSCS = new AsyncCallback<List<DDataGrid>>() {
                                ...
                                public void onSuccess(final List<DDataGrid> result1) {
                                    if (result1.size() > 0) {
                                        ...
                                        val1.setVal(1); // значение по умолчанию
                                        domain.getRows().put(new SKeyForColumn("MAIN:NUMBER_PP"), val1);
    ...
    private void showSummary() {
        ...
        map.put(4, new SKeyForColumn("MAIN:SUM_PLAN_COST"));

        DataGridPanelWithSummary summary = new DataGridPanelWithSummary("IG_OCP_PKI_ST_SUM", map, NumberFormat.getFormat("#,##0.00"), 3);
