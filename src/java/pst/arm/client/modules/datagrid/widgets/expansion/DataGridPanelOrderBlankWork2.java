 ...
 * @since 0.0.1
 */
public class DataGridPanelOrderBlankWork2 extends DataGridPanel {
    ...
    public DataGridPanelOrderBlankWork2(DTable table, String tn, DCondition cnd) {
        super(table, tn, cnd);
    }

    public DataGridPanelOrderBlankWork2(DTable table, String tn) {
        super(table, tn);
    }

    public DataGridPanelOrderBlankWork2(String tableName) {
        super(tableName);
    }

    public DataGridPanelOrderBlankWork2(String tableName, SRelationInfo relationInfo) {
        super(tableName, relationInfo);
    }

    public DataGridPanelOrderBlankWork2(DTable table, String tableName, SRelationInfo relationInfo) {
...
//        tb.add(btnCsv);
        ...
        tb.add(sprIsCh);
        btnIsCh = new Button(datagridConstants.btnAcCh());
        ...
        btnIsAll = new Button(datagridConstants.btnAcAll());
        ...
        tb.add(btnIsAll);
        tb.add(btnIsCh);
        btnDelIs = new Button(datagridConstants.btnAcDel());
    ...
    private void setIssue() {
        ...
        getService().execProc(proc, new AsyncCallback<Boolean>() {
            ...
            public void onSuccess(Boolean result) {
                if (result) {
                    MessageBox.alert("Внимание!", "Работа уже принята", null);
                }
                reloadGrid();
            }
        });
...
//                + sKeyEndDate + ")";
    ...
    private void delIssue() {
        ...
        String proc = "exec [dbo].[ORDER_BLANK_WORK_DEL] 'Принятие отозвано', " + sKeyUserId + ", " + sKeyWorkPlanId + ", 2";
        getService().execProc(proc, new AsyncCallback<Boolean>() {
            @Override
            public void onFailure(Throwable caught) {
                AppHelper.showMsgRpcServiceError(caught);
...
//        String sKeyUserId = ConfigurationManager.getUserId().toString();
//
//        String sql = "UPDATE AGREEMENT SET COMMENT='Принятие отозвано',"
...
//        }
        ...
        getReportService().generateJasperReport("OrderBlankWork2", reportType, params, condition, table, new FileReportServiceCallback(this));
