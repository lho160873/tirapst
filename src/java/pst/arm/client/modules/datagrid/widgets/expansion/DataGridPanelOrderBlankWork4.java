 ...
 * @since 0.0.1
 */
public class DataGridPanelOrderBlankWork4 extends DataGridPanel {
    ...
    public DataGridPanelOrderBlankWork4(DTable table, String tn, DCondition cnd) {
        super(table, tn, cnd);
    }

    public DataGridPanelOrderBlankWork4(DTable table, String tn) {
        super(table, tn);
    }

    public DataGridPanelOrderBlankWork4(String tableName) {
        super(tableName);
    }

    public DataGridPanelOrderBlankWork4(String tableName, SRelationInfo relationInfo) {
        super(tableName, relationInfo);
    }

    public DataGridPanelOrderBlankWork4(DTable table, String tableName, SRelationInfo relationInfo) {
...
//        tb.add(btnCsv);
        ...
        tb.add(sprIsCh);
        btnIsCh = new Button(datagridConstants.btnACh());
        ...
        btnIsAll = new Button(datagridConstants.btnAAll());
        ...
        tb.add(btnIsAll);
        tb.add(btnIsCh);
        btnDelIs = new Button(datagridConstants.btnADel());
    ...
    private void setIssue() {
        ...
        getService().execProc(proc, new AsyncCallback<Boolean>() {
            ...
            public void onSuccess(Boolean result) {
                if (result) {
                    MessageBox.alert("Внимание!", "Отчет уже принят", null);
                }
                reloadGrid();
            }
        });
...
//        SKeyForColumn key = new SKeyForColumn("AGREEMENT2:WORK_PLAN_ID");
...
//                + sKeyFactDate + ")";
    ...
    private void delIssue() {
        ...
        String proc = "exec [dbo].[ORDER_BLANK_WORK_DEL] 'Принятие отчета отозвано', " + sKeyUserId + ", " + sKeyWorkPlanId + ", 4";
        getService().execProc(proc, new AsyncCallback<Boolean>() {
            @Override
            public void onFailure(Throwable caught) {
                AppHelper.showMsgRpcServiceError(caught);
...
//            return;
//        }
//        String sKeyAgreementId = "AGREEMENT2:AGREEMENT_ID";
...
//        String sKeyUserId = ConfigurationManager.getUserId().toString();
//
//        String sql = "UPDATE AGREEMENT SET COMMENT='Принятие отчета отозвано',"
...
//        }
        ...
        getReportService().generateJasperReport("OrderBlankWork4", reportType, params, condition, table, new FileReportServiceCallback(this));
